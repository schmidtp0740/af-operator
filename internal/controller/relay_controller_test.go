...
var _ = Describe("Relay Controller", func() {
	Context("When reconciling a resource", func() {
		var (
			err                  error
			relayResource        *nodev1alpha1.Relay
			ctx                  context.Context
			typeNamespacedName   types.NamespacedName
			ns                   *v1.Namespace
			controllerReconciler *RelayReconciler
			hostPath             string
		)
		...
		BeforeEach(func() {
			...
			By("creating the custom resource for the Kind Relay")
			typeNamespacedName = types.NamespacedName{
				Name: resourceName,
				// prefix namespace with "test-relay" then apppend with a 5 character random string
				Namespace: "test-relay-" + randString(20),
			}
			...
			relayResource = &nodev1alpha1.Relay{
				...
				Spec: nodev1alpha1.RelaySpec{
					NodeSpec: nodev1alpha1.NodeSpec{
						Protocol:         "apexfusion",
						Network:          "testnet",
						LocalPeers:       []string{"af-phx-core.af-testnet.svc.cluster.local"},
						Replicas:         1,
		...
		JustBeforeEach(func() {
			...
			// Verify the Relay resource doesnt exist
			err = k8sClient.Get(ctx, typeNamespacedName, relayResource)
			Expect(err).To(HaveOccurred())
			Expect(errors.IsNotFound(err)).To(BeTrue())

			// Create the Relay resource
			Expect(k8sClient.Create(ctx, relayResource)).To(Succeed())
		...
		It("should successfully create the relay resource", func() {

			By("Reconcile to create the relay resource")
			...
			configNamespacedName := types.NamespacedName{
				Name:      "relay-" + resourceName + "-topology", // topology config map name
				Namespace: typeNamespacedName.Namespace,
			}
			...
			Expect(generatedTopologyConfig.LocalRoots[0].AccessPoints[0].Address).To(Equal("af-phx-core.af-testnet.svc.cluster.local"))
			...
			Expect(generatedTopologyConfig.PublicRoots[0].AccessPoints).To(HaveLen(2))
			Expect(generatedTopologyConfig.PublicRoots[0].AccessPoints[0].Address).To(Equal("relay-0.prime.testnet.apexfusion.org"))
			Expect(generatedTopologyConfig.PublicRoots[0].AccessPoints[0].Port).To(Equal(5521))
			Expect(generatedTopologyConfig.PublicRoots[0].AccessPoints[1].Address).To(Equal("relay-1.prime.testnet.apexfusion.org"))
			Expect(generatedTopologyConfig.PublicRoots[0].AccessPoints[1].Port).To(Equal(5521))
			Expect(generatedTopologyConfig.PublicRoots[0].Advertise).To(BeTrue())
			Expect(generatedTopologyConfig.PublicRoots[0].Valency).To(Equal(1))
			Expect(generatedTopologyConfig.UseLedgerAfterSlot).To(Equal(0))

			// check relay is created
			Eventually(func() *nodev1alpha1.Relay {
			...
			// is equal to the replicas in the relay spec
			// which should be 1
			...
			Expect(statefulSet.Labels).To(HaveKeyWithValue("instance", "relay"))
			...
			Expect(statefulSet.Spec.Selector.MatchLabels).To(HaveKeyWithValue("instance", "relay"))
			...
			Expect(statefulSet.Spec.Template.Labels).To(HaveKeyWithValue("instance", "relay"))
			...
			// Verify the args does not contain the following flags:
			// --shelley-kes-key
			// --shelley-vrf-key
			// --shelley-operational-certificate
			Expect(statefulSet.Spec.Template.Spec.Containers[0].Args).ToNot(ContainElement("--shelley-kes-key"))
			Expect(statefulSet.Spec.Template.Spec.Containers[0].Args).ToNot(ContainElement("--shelley-vrf-key"))
			Expect(statefulSet.Spec.Template.Spec.Containers[0].Args).ToNot(ContainElement("--shelley-operational-certificate"))

			// Verify the CARDANO_BLOCK_PRODUCER environment variable is not on the container
			Expect(statefulSet.Spec.Template.Spec.Containers[0].Env).To(HaveLen(0))
			...
			Expect(service.Labels).To(HaveKeyWithValue("instance", "relay"))
			...
			Expect(service.Spec.Selector).To(HaveKeyWithValue("instance", "relay"))
		...
		It("should successfully delete the relay resource", func() {
			...
			// Verify the Relay resource exists
			relayResource = &nodev1alpha1.Relay{}
			err = k8sClient.Get(ctx, typeNamespacedName, relayResource)
			Expect(err).NotTo(HaveOccurred())
			Expect(relayResource).NotTo(BeNil())
